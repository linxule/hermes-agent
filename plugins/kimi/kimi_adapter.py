...
def _ulid_time_ms(ulid_str: Optional[str]) -> Optional[int]:
    ...
    actual message ids in production are UUID v8 (per Kimi's own validation
    error: ``id_kind=uuidv8``). Their first 48 bits ARE time-monotonic but
    their units are NOT unix-ms — captured deltas show ~16× the wall-clock
    seconds, suggesting a Snowflake-style or otherwise non-standard epoch.
